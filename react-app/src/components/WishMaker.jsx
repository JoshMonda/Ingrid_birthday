...
export default function WishMaker({ onClose }) {
  ...
  return (
    <div className="fixed bottom-3 left-3 right-3 sm:bottom-5 sm:left-5 sm:right-auto z-[1000] animate-[fadeInUp_1s_ease-out] max-w-xs sm:max-w-xs mx-auto sm:mx-0">
      <div className="bg-white/95 rounded-[15px] sm:rounded-[20px] p-4 sm:p-5 text-center shadow-2xl backdrop-blur-md border-2 border-white/30 w-full">
        <div className="flex justify-between items-center mb-3 sm:mb-4">
          <h3 className="font-dancing text-xl sm:text-3xl text-green-700 m-0 pr-2">✨ Make a Christmas Wish ✨</h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-4 sm:mb-5">
          <textarea
            ...
            className="w-full h-20 sm:h-20 p-2.5 border-2 border-gray-200 rounded-[12px] sm:rounded-[15px] text-sm font-poppins resize-y outline-none transition-colors duration-300 mb-2.5 focus:border-green-600 focus:ring-4 focus:ring-green-600/10"
          />
          <button
            onClick={handleSendWish}
            className="bg-gradient-to-r from-green-600 to-red-600 text-white border-none py-3 sm:py-4 px-4 sm:px-8 rounded-[20px] sm:rounded-[25px] text-sm sm:text-lg font-medium cursor-pointer transition-all duration-300 shadow-lg hover:-translate-y-0.5 hover:shadow-xl active:translate-y-0 w-full"
          >
